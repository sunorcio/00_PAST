...
#define MAX_PACKET_SIZE_UDP 508
...
void clientSocket(void){
	...
	int status;
	SOCKET socketfd;
	struct sockaddr_in6 clientAddr = {0}, hostAddr = {0}, inboxAddr = {0};
	...
	fdSetNonblock(socketfd);


	hostAddr.sin6_family = AF_INET6;
	inet_pton(AF_INET6,"fe80::43fd:26a7:2f6e:b0bd",
			&(hostAddr.sin6_addr));
	...
	for(i = 0;i<10;i++){
		...
		fprintf(stderr,"%d/10 tries : no confirmation, "
	...
	if( !strcmp((char*)databuffer,"asdf!") ){
		printAddr6(&inboxAddr, "confirmation received, asdf!");
	}
